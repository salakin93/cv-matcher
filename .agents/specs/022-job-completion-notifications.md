...
## Estado
`DRAFT_FOR_APPROVAL` — backend only; PRD 011; depends on 004, 012 and notification/outbox foundations from 001.
...
## Alcance
### Incluido
- Notificación in-app y correo por cada transición terminal `COMPLETED`, `COMPLETED_WITH_WARNINGS`, `FAILED`, `CANCELLED` o `REAUTHORIZATION_REQUIRED` de job solicitado por usuario.
...
- Al confirmar transición terminal, `job` publica un evento interno mínimo. `notification` crea exactamente una notificación in-app y un mensaje outbox por canal/resultado/job mediante constraint de idempotencia.
- Texto español identifica título de vacante y resultado general; para completados puede referir URL interna relativa del reporte, nunca enlace público. Para `REAUTHORIZATION_REQUIRED`, indica que un `ADMIN` debe actuar, sin detalles técnicos. No incluye candidatos, scores, CVs, warning detallado, secretos ni fallo técnico.
...
- Flyway: completar `notification` con recipient, job, report_version nullable, kind, payload mínimo seguro, created/read UTC y constraint único `(recipient_user_id, job_id, kind)`; completar outbox con deduplication key por job/canal.
...
### Validación manual
- Terminar job en `COMPLETED`, `COMPLETED_WITH_WARNINGS`, `FAILED`, `CANCELLED` y `REAUTHORIZATION_REQUIRED`, y verificar una notificación y correo seguro por solicitante; para reautorización, verificar que sólo indica que un `ADMIN` debe actuar. Forzar SMTP fallido y confirmar job intacto.
