...
async function mockAuth(page: Page) { await page.route(apiRoute("/auth/**"), async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ accessToken: "access-token", tokenType: "Bearer", expiresIn: 900, user, forcePasswordChange: false }) })); }

test("un RECRUITER crea una vacante con requisitos ordenados", async ({ page }) => {
  await mockAuth(page); let body: unknown;
  await page.route(apiRoute("/vacancies"), async (route) => { body = route.request().postDataJSON(); await route.fulfill({ status: 201, contentType: "application/json", body: JSON.stringify(detail) }); });
...
test("lista archivadas y confirma archivo", async ({ page }) => {
  await mockAuth(page); await page.route(apiRoute("/vacancies?**"), async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ items: [{ id: "vacancy-id", title: "Backend Java", status: "ACTIVE", version: 2, updatedAt: "2026-09-01T12:00:00Z" }], page: 0, size: 20, totalItems: 1, totalPages: 1 }) })); await page.route(apiRoute("/vacancies/vacancy-id"), async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(detail) })); await page.route(apiRoute("/vacancies/vacancy-id/archive"), async (route) => route.fulfill({ status: 204 }));
...
test("un 403 al archivar descarta el detalle ya cargado", async ({ page }) => {
  await mockAuth(page); await page.route(apiRoute("/vacancies/vacancy-id"), async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(detail) })); await page.route(apiRoute("/vacancies/vacancy-id/archive"), async (route) => route.fulfill({ status: 403, contentType: "application/json", body: JSON.stringify({ status: 403, code: "FORBIDDEN", message: "Interno" }) })); await page.route(apiRoute("/vacancies?**"), async (route) => route.fulfill({ status: 403, contentType: "application/json", body: JSON.stringify({ status: 403, code: "FORBIDDEN", message: "Interno" }) }));
...
test("reactiva con expectedVersion y el diálogo se puede controlar con teclado", async ({ page }) => {
  ...
  await page.route(apiRoute("/vacancies/vacancy-id"), async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(archived) })); await page.route(apiRoute("/vacancies/vacancy-id/reactivate"), async (route) => { body = route.request().postDataJSON(); await route.fulfill({ status: 204 }); });
...
for (const [status, code, message] of [[401, "UNAUTHENTICATED", "Bienvenido"], [403, "FORBIDDEN", "No tienes permisos"], [404, "VACANCY_NOT_FOUND", "ya no está disponible"], [422, "VALIDATION_ERROR", "Revisa los campos"], [409, "VACANCY_ARCHIVED", "No se puede editar"]] as const) {
  test(`maneja ${code} de forma segura`, async ({ page }) => {
    await mockAuth(page); await page.route(apiRoute("/vacancies/vacancy-id"), async (route) => route.fulfill({ status, contentType: "application/json", body: JSON.stringify({ status, code, message: "Interno" }) })); await page.goto("/vacantes/vacancy-id"); if (status === 401) await expect(page.getByRole("heading", { name: "Bienvenido" })).toBeVisible(); else if (status === 403) { await expect(page).toHaveURL(/\/vacantes$/); await expect(page.getByText("Backend Java")).not.toBeVisible(); } else await expect(page.getByRole("alert")).toContainText(message);
  });
}
